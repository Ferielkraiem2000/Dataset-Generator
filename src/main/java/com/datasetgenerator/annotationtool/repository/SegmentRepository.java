...
public interface SegmentRepository extends JpaRepository<Segment, Long> {
    ...
    @Query("SELECT f.file_name, SUM(s.duration), AVG(s.duration), COUNT(s.segment_id), COUNT(DISTINCT s.speaker), f.upload_time FROM Segment s JOIN s.file f WHERE f.file_id IN :fileIds group by f.file_id")
